<script>
  ...
  let viewportH = 0;
  let logoTransform = "translate3d(-50%, -50%, 0)";
  let logoScale = "scale(1)";
  ...
  function updateLogoState() {
    ...
    // mouvement étalé plus longtemps sur mobile pour éviter la fin brutale
    const moveWindow = isMobile ? 0.27 : 0.2;

    const normalized = phase === "done"
      ? clamp(raw / moveWindow, 0, 1)
      : 0;

    const logoProgress = easeOutQuart(normalized);

    applyLogoProgress(logoProgress);
  }
  ...
  onMount(() => {
    ...
    const doneDelay = isMobile ? 6100 : 5200;
...
>
  <div class="logo-mover" style="transform:{logoTransform};">
    <div class="logo-scaler" style="transform:{logoScale};">
...
<style>
  ...
  @media (max-width: 768px) {
    ...
    .eagle.play path {
      animation: draw 5.55s cubic-bezier(.7,0,.3,1) forwards;
    }
    ...
    .progress-bar.play {
      animation: loading 5.7s cubic-bezier(.7,0,.3,1) forwards;
    }
  }
</style>
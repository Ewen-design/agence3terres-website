<script>
  ...
  function updateHero() {
    ...
    let raw = max > 0 ? clamp(-rect.top / max, 0, 1) : 0;
    ...
    const normalizedLogo = clamp(progress / logoWindow, 0, 1);
    logoProgress = easeOutQuart(normalizedLogo);
...
<style>
  ...
  .ambient { opacity: calc(0.025 + var(--p) * 0.045); will-change: transform, opacity; }
  ...
  .hero-logo-mover {
    ...
    top: 50%;
    left: 50%;
    transform: translate3d(-50%, calc(-50% - var(--lp) * 24vh), 0);
    will-change: transform;
  }
  ...
  .word {
    ...
    transition: opacity 2s cubic-bezier(.22,.61,.36,1), transform 2s cubic-bezier(.22,.61,.36,1), filter 2s cubic-bezier(.22,.61,.36,1);
  }
  ...
  @keyframes baseDrift { 0% { transform: scale(1) translate3d(-0.6%, -0.4%, 0); } 100% { transform: scale(1.04) translate3d(0.6%, 0.4%, 0); } }
  @keyframes ambientOne { 0% { transform: translate3d(-1.2%, -0.9%, 0) scale(1.01); } 100% { transform: translate3d(1.3%, 0.9%, 0) scale(1.04); } }
  @keyframes ambientTwo { 0% { transform: translate3d(1.1%, -1.1%, 0) scale(1.01); } 100% { transform: translate3d(-1.3%, 1.1%, 0) scale(1.04); } }
  @keyframes ambientThree { 0% { transform: translate3d(-0.9%, 1.1%, 0) scale(1.02); } 100% { transform: translate3d(1%, -1.2%, 0) scale(1.05); } }
  ...
  @media (max-width: 768px) {
    ...
    .base-gradient { background: radial-gradient(112% 88% at 50% 92%, rgba(255,249,240,calc(0.18 + var(--p) * 0.84)) 0%, rgba(255,236,214,calc(0.18 + var(--p) * 0.74)) 18%, rgba(241,199,146,calc(0.16 + var(--p) * 0.62)) 38%, rgba(214,126,60,calc(0.18 + var(--p) * 0.50)) 58%, rgba(108,40,12,calc(0.42 + var(--p) * 0.28)) 78%, rgba(5,2,2,1) 100%); }
    ...
    .hero-logo-mover {
      transform: translate3d(-50%, calc(-50% - var(--lp) * 19vh), 0);
    }

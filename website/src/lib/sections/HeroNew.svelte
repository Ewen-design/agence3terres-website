<script>
  ...
  function snapTo(direction) {
    ...
    // laisse le scroll normal continuer hors des bords
    if (direction === "down" && raw >= 0.94) return;
    if (direction === "up" && raw <= 0.06) return;
  ...
  function handleWheel(e) {
    ...
    if (e.deltaY > 8 && raw < 0.94) {
      e.preventDefault();
      snapTo("down");
    } else if (e.deltaY < -8 && raw > 0.06) {
      e.preventDefault();
      snapTo("up");
    }
  }
  ...
  function handleTouchMove(e) {
    ...
    if (delta < -16 && raw < 0.94) {
      e.preventDefault();
      snapTo("down");
    } else if (delta > 16 && raw > 0.06) {
      e.preventDefault();
      snapTo("up");
    }
  }
  ...
  function updateHero() {
    ...
    // pour le fond / texte
    const easedRaw = easeOutCubic(raw);
    smoothProgress += (easedRaw - smoothProgress) * 0.07;
    progress = smoothProgress;

    // pour le logo : direct, sans lissage parasite ni snap final
      ...
      <div class="hero-logo-mover">
        <div class="hero-logo-scaler">
          <svg viewBox="0 0 623.51 321.98" class="hero-eagle">
            <!-- Remets ici exactement le même SVG complet que dans le preloader -->
